...
export async function apiFetch(endpoint, options = {}) {
  // 1. Format URL endpoint agar bersih dari double slash
  ...
  if (isInternalRoute) {
    ...
    // Jika rute lain (seperti /api/auth/me) membutuhkan token jika tersedia
    defaultHeaders['Authorization'] = `Bearer ${token}`;
  }
...
export const internalLabService = {
  ...
  updateStatus: (id, updateData) => {
    // updateData: { status_uji, hasil_dokumen_url, keterangan, tanggal_selesai }
...
export const benihService = {
  getAllPublic: (params = {}) => {
    const query = new URLSearchParams(params).toString();
    return api.get(`/api/public/benih${query ? `?${query}` : ''}`);
  },
  getByIdPublic: (id) => api.get(`/api/public/benih/${id}`),
};
